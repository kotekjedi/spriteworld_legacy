...
class Beta(AbstractDistribution):
  ...
  def __init__(self, key, alpha, beta, dtype='float32'):
    """Construct continuous 1-dimensional uniform distribution.
    Args:
      key: String factor name. self.sample() returns {key: _}.
      minval: Scalar minimum value.
    ...
    self.dtype = dtype
    self.counter = -2
    self.factor_values = np.zeros((30000,6))
  ...
  def sample(self,rng=None):
    ...
    if self.counter > -1:
      if self.key == 'x':
        self.factor_values[self.counter][0]

      elif self.key == 'y':
        self.factor_values[self.counter][1]

      elif self.key == 'scale':
        self.factor_values[self.counter][2]

      elif self.key == 'angle':
        self.factor_values[self.counter][3]
      elif self.key == 'c0':
        self.factor_values[self.counter][4]
